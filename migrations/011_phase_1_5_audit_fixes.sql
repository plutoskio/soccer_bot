...
SET maximum_attempts = 1
WHERE maximum_attempts IS NULL;

UPDATE collection_checkpoint
SET priority = 2
WHERE priority IS NULL;

DROP INDEX IF EXISTS collection_checkpoint_due_idx;
DROP INDEX IF EXISTS collection_checkpoint_schedule_idx;
DROP INDEX IF EXISTS collection_checkpoint_fixture_component_idx;
...
result_quality AS (
    SELECT
        f.fixture_id,
        coalesce(
            ls.canonical_status = 'administrative_result',
            f.status IN ('administrative_result_unplayed', 'AWD', 'WO'),
            false
        ...
        AND (
            ls.canonical_status IS NULL
            OR ls.canonical_status = 'final'
        ) AS result_ok
    FROM fixture f
    LEFT JOIN fixture_result_observation r USING (fixture_id)
    LEFT JOIN latest_schedule ls USING (fixture_id)
    GROUP BY f.fixture_id, f.status, ls.canonical_status
),
team_artifact AS (
    SELECT
        tm.fixture_id,
...
flags AS (
    SELECT
        ...
        rq.result_ok AND NOT rq.administrative_unplayed
            AS eligible_result_models,
        rq.result_ok AND NOT rq.administrative_unplayed
            AND coalesce(tq.team_ok, false) AS eligible_team_models,
        rq.result_ok AND NOT rq.administrative_unplayed
            AND coalesce(pq.player_ok, false) AS eligible_player_models
    FROM fixture f
